...
@app.route('/')
def home():
    return "Bot je online a běží!", 200
...
def background_email_collector_job(days_to_fetch=1):
    ...
    try:
        ...
        print(f"Nalezeno {total_msgs} zpráv k zpracování.", flush=True)
...
Zpracuj níže uvedenou komunikaci / téma do **maximálně podrobného, přesného a strukturovaného přehledu**.
...
Data k analýze:
{'\n'.join(emails_text) if emails_text else 'Zpracuj primárně ze svých znalostí aktuálních tržních trendů a veřejných zdrojů.'}
"""
    try:
...
def process_command(command, chat_id):
    ...
    if "sales" in cmd:
        ...
        send_telegram_message(chat_id, f"📊 Generuji oborový report pro DE/AT trh z veřejných zdrojů...")
        analysis = analyze_with_openai([], "Oborový přehled trhu železniční nákladní dopravy (DE/AT): Změny cen energií, poplatky za dopravní cestu (Trassengebühren), výluky DB InfraGO & ÖBB, zprávy z odborových svazů a trhu práce (mzdy). Vždy uveď zdroje a odkazy na články online.")
        send_telegram_message(chat_id, analysis[:4000])
        return
...
3. **⚠️ Akční kroky & Doporučené reakce**: U každého kritického bodu jasně zformuluj doporučenou reakci a další krok.
"""
