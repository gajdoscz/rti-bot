        ...
        for e_id in reversed(email_ids):
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    combined_content = f"Od: {sender} | Pro: {to_field} | Kopie (Cc): {cc_field}\nPředmět: {subject}\nObsah: {body[:400]}...\n{attachments_text}\n---"
...
2. **Sales & Obchodní poptávky (včetně dat z příloh):** Pečlivě analyzuj data z textů i přiložených tabulek. Seskup objednávky a poptávky přehledně podle zákazníků. U každého zákazníka uveď seznam požadovaných vlaků, tras/relací z tabulek a celkový přesný počet poptávaných vlaků.
3. **Stručnost a věcnost:** Žádná vata, formou úderných odrážek.

Režim: {mode_description}

E-maily a přílohy k analýze:
{'\n'.join(emails_text)}
"""
    try:
